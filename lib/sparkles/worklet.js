if (typeof registerPaint !== 'undefined') {
  class ExtraSparkles {
    ...
    paint(ctx, size, properties) {
      const sparkles = properties.get('--extra-sparkleNumber') || 30
      const minHeight = 3
      const maxHeight = minHeight + parseInt(properties.get('--extra-sparkleHeightVariance')) || 9
      const minWidth = 3
      const maxWidth = minWidth + parseInt(properties.get('--extra-sparkleWidthVariance')) || 12
      const minWeight = 1
      const maxWeight = minWeight + parseInt(properties.get('--extra-sparkleWeightVariance')) || 2
      ...
      for (let i = 0; i < sparkles; i++) {
        ...
        // Set Color
        const hueVal = parseInt(properties.get('--extra-sparkleHue')) || 30
